// OrbitImages — adapted from React Bits / Dominik Koch (MIT-style community component)
...
export default function OrbitImages({
  ...
  const baseHeight = baseHeightIn ?? baseWidth
  ...
  const path = useMemo(() => {
    switch (shape) {
      ...
      case 'infinity':
        return getInfinityPathD(baseWidth, baseHeight)
      case 'wave':
  ...
  }, [shape, customPath, baseWidth, baseHeight, designCenterX, designCenterY, radiusX, radiusY, radius, starPoints, starInnerRatio])
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      const sw = el.clientWidth / baseWidth
      const sh = el.clientHeight / baseHeight
      setScale(Math.max(Math.min(sw, sh), 0.0001))
    }
    updateScale()
    const observer = new ResizeObserver(updateScale)
    observer.observe(containerRef.current)
    return () => observer.disconnect()
  }, [responsive, baseWidth, baseHeight])
  ...
  return (
    <div
      ...
      style={{
        ...
        aspectRatio: responsive ? `${baseWidth} / ${baseHeight}` : undefined,
      }}
      aria-hidden="true"
    >
      <div
      ...
      >
        <div className="orbit-rotation-wrapper" style={{ transform: `rotate(${rotation}deg)` }}>
          {showPath && (
            <svg width="100%" height="100%" viewBox={`0 0 ${baseWidth} ${baseHeight}`} className="orbit-path-svg">
              <path d={path} fill="none" stroke={pathColor} strokeWidth={pathWidth / safeScale} />
            </svg>
          )}
